...
class TestDeleteConfirmation:
    ...
    async def test_handle_delete_shows_dialog_with_title(self):
        rk = cache.put({"category": "десерт", "slug": "tiramisu"})
        cb = _make_callback(f"del:d:{rk}")
    ...
    @pytest.mark.asyncio
    async def test_handle_delete_falls_back_to_slug_when_no_recipe(self):
        rk = cache.put({"category": "десерт", "slug": "tiramisu"})
        cb = _make_callback(f"del:d:{rk}")
...
class TestDeleteConfirm:
    ...
    async def test_confirm_removes_recipe(self):
        rk = cache.put({"category": "десерт", "slug": "tiramisu"})
        cb = _make_callback(f"delok:d:{rk}")

        with patch.object(gm, "get_user_active_group", return_value="g1") as m_group, \
                patch.object(gm, "remove_recipe_from_group", return_value=True) as m_remove, \
        ...
        m_remove.assert_called_once_with("g1", "десерт", "tiramisu")
        m_delete.assert_called_once_with("десерт", "tiramisu")
        text = cb.message.edit_text.call_args.args[0]
    ...
    async def test_confirm_keeps_recipe_when_in_other_group(self):
        ...
        with patch.object(gm, "get_user_active_group", return_value="g1"), \
                patch.object(gm, "remove_recipe_from_group", return_value=True), \
...
class TestDeleteCancel:
    ...
    async def test_cancel_restores_recipe_view(self):
        rk = cache.put({"category": "десерт", "slug": "tiramisu"})
        cb = _make_callback(f"delcn:d:{rk}")

        with patch.object(
            gm,
            "get_recipe",
            return_value={"title": "Тирамису", "content_md": RECIPE_MD, "source": "ig"},
        ), patch.object(gm, "find_source_by_slug", return_value=None):
